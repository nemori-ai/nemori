...
def get_client(frame: str, user_id: str | None = None, version: str = "default"):
    if frame == "zep":
        ...
        mem_cube_config_data["text_mem"]["config"]["graph_db"]["config"][
            "db_name"
...
def process_user(conv_idx, frame, locomo_df, version, num_workers=1):
    try:
        ...
        print(f"Processing {valid_sessions} sessions for user {conv_idx} with {num_workers} workers")
        with concurrent.futures.ThreadPoolExecutor(max_workers=num_workers) as executor:
            futures = {
                executor.submit(ingest_session, client, session, frame, metadata, revised_client): metadata[
                    "session_key"
                ]
