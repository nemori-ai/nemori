...
class NemoriExperiment:
    ...
    def convert_conversation_to_nemori(self, conversation_data: dict) -> RawEventData:
        ...
        return RawEventData(
            ...
            temporal_info=TemporalInfo(
                ...
                duration=duration,
                #timezone="UTC",
                timezone="Asia/Shanghai",# zh version
            ),
    ...
    def convert_locomo_to_nemori(self, conversation_data: dict, conversation_id: str) -> RawEventData:
        ...
        if messages:
            ...
            first_time = datetime.now(shanghai_tz)
            #first_time = datetime.now()
        #temporal_info = TemporalInfo(timestamp=first_time, duration=duration, timezone="UTC")
        temporal_info = TemporalInfo(timestamp=first_time, duration=duration, timezone="Asia/Shanghai")
            ...
            if session_time_key in conv:
                ...
                for i, msg in enumerate(session_messages):
                    if "timestamp" in msg:
                        ...
                        # 只有当时间戳以Z结尾时才替换为+00:00，其他情况保持原样
                    ...
                    if speaker_name not in speaker_name_to_id:
                        # Generate unique ID: {name}_{conversation_index}
                        unique_id = f"{speaker_name.lower().replace(' ', '_')}_{conversation_id}"
                        # if speaker_name.lower() == conversation_id.lower():
                        #     # If speaker name matches conversation ID, just use the speaker name
                        #     unique_id = f"{speaker_name.lower().replace(' ', '_')}"
                        # else:
                        #     # Otherwise use the original format: {name}_{conversation_index}
                        #     unique_id = f"{speaker_name.lower().replace(' ', '_')}_{conversation_id}"
    ...
    async def perform_complete_semantic_discovery(self, episode, segment_messages, segment_id):
        ...
        original_conversation = "\n".join([f"{msg.user_name}: {msg.content}" for msg in segment_messages])
        discovered_count = 0
    ...
    async def _build_episodes_semantic_speaker_mode(self, raw_data: RawEventData) -> list:
        ...
        # speaker_a_name = metadata.get("speaker_a")
        # speaker_b_name = metadata.get("speaker_b")
        # speakers = set()
        # if speaker_a_name:
        #     speakers.add(f"{str(speaker_a_name).lower()}")
        # if speaker_b_name:
        #     speakers.add(f"{str(speaker_b_name).lower()}")
        print(f"   👥 Speakers: {list(speakers)}")
        print(f"   💬 Messages: {len(raw_data.content)}")
        print(f"   🕐 Duration: {raw_data.temporal_info.duration:.0f} seconds")
