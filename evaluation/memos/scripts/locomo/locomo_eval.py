...
# Download necessary NLTK resources (lazy loading for quick mode)
...
# Lazy loading: Only load when needed (not in quick mode)
sentence_model = None
...
def calculate_semantic_similarity(gold_answer, response):
    global sentence_model, sentence_model_name
...
def calculate_nlp_metrics(gold_answer, response, context, options=None, quick_mode=False):
    if options is None:
    ...
    if "semantic" in options:
        ...
        _, _, f1 = bert_score([gold_answer], [response], lang="en", rescale_with_baseline=True, verbose=False)
...
async def process_group_responses(group_id, group_responses, oai_client, options, num_runs: int, quick_mode=False):
    graded_responses = []
    ...
    for response in tqdm(group_responses, desc=f"Processing group {group_id}"):
        ...
        nlp_metrics = calculate_nlp_metrics(ground_truth, answer, context, options, quick_mode=quick_mode)
...
async def process_single_group(group_id, group_responses, oai_client, options, num_runs, quick_mode=False):
    try:
        start_time = time.time()
        result = await process_group_responses(group_id, group_responses, oai_client, options, num_runs, quick_mode=quick_mode)
        end_time = time.time()
...
async def main(frame, version="default", options=None, num_runs=1, max_workers=4, quick_mode=False):
    mode_text = "QUICK MODE (LLM-only)" if quick_mode else "FULL MODE (LLM + NLP metrics)"
    ...
    for group_idx in range(num_users):
        ...
        active_users += 1
        tasks.append(process_single_group(group_id, group_responses, oai_client, options, num_runs, quick_mode=quick_mode))
...
if __name__ == "__main__":
    ...
    parser.add_argument("--workers", type=int, default=100, help="Number of concurrent workers for processing groups")
    parser.add_argument("--quick", action="store_true", help="Enable quick mode (LLM evaluation only, skip NLP metrics)")
    args = parser.parse_args()

    asyncio.run(main(args.lib, args.version, args.options, args.num_runs, args.workers, args.quick))
