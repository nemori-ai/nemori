...
def calculate_scores(data):
    ...
    for user, questions in data.items():
        ...
        user_metrics[user]["context_tokens"] = (
            np.mean(user_metrics[user]["context_tokens"]) if user_metrics[user]["context_tokens"] else 0.0
        )
    ...
    for category, score_data in category_scores.items():
        ...
        category_overall_scores[category] = {
            ...
            "llm_judge_std": np.std(category_judgment_avgs) if len(category_judgment_avgs) > 1 else 0.0,
        ...
        for group in ["lexical", "semantic"]:
            for metric in category_metrics[category][group]:
                ...
                category_overall_scores[category][group][metric] = np.mean(values) if values else 0.0
        ...
        duration_metrics = list(category_metrics[category]["duration"].keys())  # Create a list of keys first
        for metric in duration_metrics:
            ...
            if values:
                ...
                category_overall_scores[category]["duration"][f"{metric}_p50"] = np.percentile(values, 50)
                category_overall_scores[category]["duration"][f"{metric}_p95"] = np.percentile(values, 95)
            else:
...
print(f"LLM-as-a-Judge score: {results['metrics']['llm_judge_score']:.4f} ± {results['metrics']['llm_judge_std']:.4f}")
