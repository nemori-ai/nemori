...
async def main(frame, version="default"):
    ...
    load_dotenv()
    oai_client = AsyncOpenAI(api_key=os.getenv("CHAT_MODEL_API_KEY"), base_url=os.getenv("CHAT_MODEL_BASE_URL"))
    ...
    for group_idx in range(num_users):
        ...
        for qa in qa_set_filtered:
            ...
            matching_result = next((result for result in search_results if result.get("query") == question), None)
            if matching_result:
