...
from ..core.data_types import ConversationData, RawEventData
...
class EnhancedConversationEpisodeBuilder(ConversationEpisodeBuilder):
    ...
    async def build_episode(self, data: RawEventData, for_owner: str) -> Episode:
        ...
        if self.semantic_manager and isinstance(data, ConversationData):
            ...
            # 发现并存储语义知识
            semantic_nodes = await self.semantic_manager.process_episode_for_semantics(episode, original_content)

            # Add semantic metadata to episode
            # 向情景添加语义元数据
            if semantic_nodes:
                episode.metadata.custom_fields["discovered_semantics"] = len(semantic_nodes)
                episode.metadata.custom_fields["semantic_node_ids"] = [node.node_id for node in semantic_nodes]
        ...
        try:
            ...
            return {
                "discovered_semantic_nodes": [
                    {"key": node.key, "value": node.value, "confidence": node.confidence} for node in discovered_nodes
                ],
                "linked_semantic_nodes": [
                    {"key": node.key, "value": node.value, "confidence": node.confidence}
                    for node in linked_nodes
                    if node.node_id not in [n.node_id for n in discovered_nodes]
                ],
    ...
    def get_semantic_summary(self, semantic_nodes: list[Any]) -> str:
        """
        ...
        knowledge_summary = []
        for node in semantic_nodes:  # Top 5 nodes [:5]
            knowledge_summary.append(f"{node.key}: {node.value}...")
