...
class MemorySystem:
    ...
    async def add_messages(self, user_id: str, messages: list[Message]) -> None:
        ...
        await self._buffer_store.push(user_id, messages)
        count = await self._buffer_store.count_unprocessed(user_id)
    ...
    async def _process(self, user_id: str) -> list[Episode]:
        ...
        async with self._get_lock(user_id):
            messages = await self._buffer_store.get_unprocessed(user_id)
            if not messages:
                return []
            ...
            for group in groups:
                ...
                episode = await self._episode_gen.generate(
                    user_id, group_msgs, group.get("topic", "conversation")
                )
            ...
            if buffer_ids:
                await self._buffer_store.mark_processed(user_id, buffer_ids)
    ...
    async def _on_episode_created(self, user_id: str, episode: Episode) -> None:
        ...
        try:
            existing_eps = await self._episode_store.list_by_user(user_id, limit=20)
            existing_sem = await self._semantic_store.list_by_user(user_id)

            memories = await self._semantic_gen.generate(
                user_id, episode, existing_eps, existing_sem
            )
            if memories:
    ...
    async def delete_episode(self, user_id: str, episode_id: str) -> None:
        await self._episode_store.delete(episode_id)

    async def delete_semantic(self, user_id: str, memory_id: str) -> None:
        await self._semantic_store.delete(memory_id)

    async def delete_user(self, user_id: str) -> None:
        await self._semantic_store.delete_by_user(user_id)
        await self._episode_store.delete_by_user(user_id)

    async def stats(self, user_id: str) -> dict[str, Any]:
        episodes = await self._episode_store.list_by_user(user_id)
        semantics = await self._semantic_store.list_by_user(user_id)
        buffer_count = await self._buffer_store.count_unprocessed(user_id)
        return {
            "user_id": user_id,
