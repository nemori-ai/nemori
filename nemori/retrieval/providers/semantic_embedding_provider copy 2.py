...
class SemanticEmbeddingProvider:
    ...
    async def _rebuild_semantic_index(self, owner_id: str) -> None:
        ...
        semantic_nodes = await self.storage_repo.get_all_semantic_nodes_for_owner(owner_id)
    ...
    async def _reload_semantic_from_storage(self, owner_id: str) -> None:
        """Reload semantic nodes from storage repository for the given owner."""
        try:
            if not self.storage_repo:
                print("Warning: No storage repository available to reload semantic nodes")
                return

            # Get semantic nodes for this owner from storage
            semantic_nodes = await self.storage_repo.get_all_semantic_nodes_for_owner(owner_id)

            if not semantic_nodes:
                print(f"Warning: No semantic nodes found in storage for owner: {owner_id}")
                return

            # Get the user index
            index = self._get_user_index(owner_id)

            # Initialize semantic_nodes array to match embeddings size (maintain synchronization)
            embeddings_size = len(index["embeddings"])
            index["semantic_nodes"] = [None] * embeddings_size

            # Re-add semantic nodes in the same order as the embeddings
            nodes_found = 0
            for node in semantic_nodes:
                if node.node_id in index["node_id_to_index"]:
                    embeddings_index = index["node_id_to_index"][node.node_id]
                    if 0 <= embeddings_index < embeddings_size:
                        index["semantic_nodes"][embeddings_index] = node
                        nodes_found += 1

            print(f"Reloaded {nodes_found} semantic nodes for owner: {owner_id} (embeddings size: {embeddings_size})")

            # Verify synchronization
            if nodes_found != embeddings_size:
                print(f"Warning: Semantic nodes count ({nodes_found}) doesn't match embeddings size ({embeddings_size})")

        except Exception as e:
            print(f"Error reloading semantic nodes from storage for {owner_id}: {e}")
