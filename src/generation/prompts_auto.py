...
You are a dialogue boundary detection expert. You need to determine if the newly added dialogue should end the current episode and start a new one. The language of your entire response, including the reason and summary, MUST match the language of the conversation.
...
Please return your judgment as a raw JSON object. Do not use markdown code blocks or any other formatting.
{{
    "should_end": true/false,
    "reason": "Specific reason for the judgment, written in the same language as the conversation",
    "confidence": 0.0-1.0,
    "topic_summary": "If ending, summarize the core topic of the current episode, written in the same language as the conversation"
}}
...
You are an episodic memory generation expert. Please convert the following conversation into an episodic memory. The `title` and `content` fields in your response MUST be in the same language as the input conversation.
...
Please analyze the conversation to extract time information and generate a structured episodic memory. Return ONLY a raw JSON object without any explanations, markdown formatting, or code blocks.
{{
    "title": "A concise, descriptive title that accurately summarizes the theme (10-20 words), in the conversation's language",
    "content": "A detailed description of the conversation in third-person narrative, written in the conversation's language. It must include all important information: who participated in the conversation at what time, what was discussed, what decisions were made, what emotions were expressed, and what plans or outcomes were formed. Write it as a coherent story so that the reader can clearly understand what happened. Ensure that time information is precise to the hour, including year, month, day, and hour.",
...
"""
    ...
    PREDICTION_PROMPT = """
You are a knowledge-based episode prediction system. Your task is to reconstruct a complete conversation episode based on limited clues and your knowledge base. The language of your response MUST match the language of the input clues.
...
## Output Format
Generate a natural narrative that captures what you predict happened. Write it as if you're describing the episode to someone else. Focus on the SUBSTANCE, not the STYLE. Do not use code blocks or any markdown formatting.
...
You are extracting valuable knowledge by comparing original conversation with predicted content. The extracted statements MUST be in the same language as the `Original Conversation`.
...
## Output Format:
Return ONLY the raw JSON object. Do not add any extra text, explanations, or markdown code blocks.
{{
    "statements": [
...
You are an AI memory system. Extract HIGH-VALUE, PERSISTENT semantic memories from the following episodes. All extracted `statements` MUST be in the same language as the input `episodes`.
...
## Output Format
Return ONLY a raw JSON object, without any surrounding text, explanations, or markdown formatting.
{{
    "statements": [
