...
class TestSemanticMemory:
    ...
    async def test_semantic_discovery(self, semantic_engine):
        ...
        discovered_nodes = await semantic_engine.discover_semantic_knowledge(episode, original_content)
    ...
    async def test_semantic_evolution(self, evolution_manager, semantic_storage):
        ...
        processed_nodes = await evolution_manager.process_episode_for_semantics(episode, original_content)
    ...
    async def test_unified_retrieval_service(self, unified_retrieval, semantic_storage):
        ...
        nodes = [
            SemanticNode(
                owner_id="test_user", key="John的研究方向", value="AI Agent行为规划", context="专注于决策机制"
            ),
            SemanticNode(
                owner_id="test_user", key="项目进展", value="Tanka项目已完成原型", context="用户界面设计已确定"
            ),
        ]
        ...
        results = await unified_retrieval.search_semantic_memories(owner_id="test_user", query="研究", limit=10)
